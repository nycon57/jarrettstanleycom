...
{ trackingData, onClick, children, external = false, href, ref, ...props }: TrackedLinkProps & {ref?: React.Ref<HTMLAnchorElement>;}) {
  const { trackLink } = useAnalytics();

  const trackLinkActivation = (event: React.MouseEvent<HTMLAnchorElement>) => {
  ...
  return (
    <Link ref={ref} href={href} onClick={trackLinkActivation} {...props}>
        {children}
      </Link>);
